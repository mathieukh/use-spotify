...
import { Artist, IncludeGroup } from "./types";
...
export class ArtistsApi {
  ...
  public static fromClient(spotifyClient: AxiosInstance) {
    return new ArtistsApi(spotifyClient);
  }
  ...
  getArtist = async ({ id }: { id: string }) => {
    const response = await this.spotifyClient.get(`/artists/${id}`);
   ...
   */
  getArtists = async ({ ids }: { ids: string[] }) => {
    const response = await this.spotifyClient.get(`/artists`, {
      params: {
        ids: ids.join(","),
      },
    });
  ...
  getArtistAlbums = async ({
    ...
    const response = await this.spotifyClient.get(`/artists/${id}/albums`, {
      params: {
        include_groups,
        ...params,
      },
  ...
  getArtistTopTracks = async ({
    ...
    const response = await this.spotifyClient.get(`/artists/${id}/top-tracks`, {
      params,
    });
   ...
   */
  getRelatedArtists = async ({ id }: { id: string }) => {
